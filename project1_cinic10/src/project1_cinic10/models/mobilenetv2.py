...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes: int = 10, dropout: float = 0):
        ...
        self.stem = nn.Sequential(
            ...
            ),  # Changed stride from 2 -> 1 to adjust for 32x32 input image
            nn.BatchNorm2d(32),
            nn.ReLU6(inplace=True),
        )
        self.in_channels = 32
    ...
    def _make_stage(self, t: int, c: int, n: int, s: int) -> list[nn.Module]:
        ...
        """
        blocks = [
            InvertedResidual(self.in_channels, c, s, t)
        ]  # first block: transition
        self.in_channels = c  # update for subsequent blocks
        for _ in range(n - 1):
            blocks.append(
                InvertedResidual(c, c, 1, t)
            )  # rest: same channels, stride 1, that means residual connections
        return blocks
