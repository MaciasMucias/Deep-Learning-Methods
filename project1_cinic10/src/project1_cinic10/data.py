...
class Cutout:
    ...
    def __call__(self, img: torch.Tensor) -> torch.Tensor:
        h, w = img.shape[-2:]
        if h < self.size:
            raise ValueError(f"Image too short ({h} < {self.size})")
        if w < self.size:
            raise ValueError(f"Image too narrow ({w} < {self.size})")
...
def build_transforms(
    ...
    if random_crop:
        transforms_list.append(
            transforms.RandomCrop(size=crop_size, padding=crop_padding)
        )
    if rotation:
...
class PreloadedDataset(Dataset):
    def __init__(
        ...
        preload_transform = transforms.Compose(
            transform.transforms[: to_tensor_idx + 1]
        )
        self.transform = (
            transforms.Compose(transform.transforms[to_tensor_idx + 1 :])
            if transform.transforms[to_tensor_idx + 1 :]
            else None
        )
